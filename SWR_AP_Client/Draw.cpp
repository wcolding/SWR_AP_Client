...
namespace SWRGame 
{
	...
	void __fastcall DrawEvents::OnDrawPreRaceMenu()
	{
		...
		availableShopChecksStr = std::format(" - {} checks ", shopManager.GetAvailableShopChecks(completedCourses));
		WriteTextWrapper(availableShopChecksStr, SWRFont::ShopItem, 110, 110, SWRTextColor::AP_ProgressionItem);

		int pitDroidChecksAvailable = 4 - SWRGame::progress.pitDroidCounter;
		if (pitDroidChecksAvailable > 0)
		{
			availablePitDroidChecksStr = std::format(" - {} checks ", pitDroidChecksAvailable);
			WriteTextWrapper(availablePitDroidChecksStr, SWRFont::ShopItem, 140, 120, SWRTextColor::AP_ProgressionItem);
		}
	}
